...
{
	if (strcmp(node.name(),"line")==0) { m_line = std::make_unique<t_road_planView_geometry_line>(node); }
	if (strcmp(node.name(),"spiral")==0) { m_spiral = std::make_unique<t_road_planView_geometry_spiral>(node); }
	if (strcmp(node.name(),"arc")==0) { m_arc = std::make_unique<t_road_planView_geometry_arc>(node); }
	if (strcmp(node.name(),"poly3")==0) { m_poly3 = std::make_unique<t_road_planView_geometry_poly3>(node); }
	if (strcmp(node.name(),"paramPoly3")==0) { m_paramPoly3 = std::make_unique<t_road_planView_geometry_paramPoly3>(node); }
}
t_road_lanes_laneSection_lr_lane_U::t_road_lanes_laneSection_lr_lane_U(pugi::xml_node node)
{
	for (pugi::xml_node e_width : node.children())
	{
		...
		{
			m_widths.push_back(std::make_unique<t_road_lanes_laneSection_lr_lane_width>(e_width));
		}
	}
	for (pugi::xml_node e_border : node.children())
	{
		...
		{
			m_borders.push_back(std::make_unique<t_road_lanes_laneSection_lr_lane_border>(e_border));
		}
	}
}
t_road_signals_signal_U::t_road_signals_signal_U(pugi::xml_node node)
{
	if (strcmp(node.name(),"positionRoad")==0) { m_positionRoad = std::make_unique<t_road_signals_signal_positionRoad>(node); }
	if (strcmp(node.name(),"positionInertial")==0) { m_positionInertial = std::make_unique<t_road_signals_signal_positionInertial>(node); }
}
t_header::t_header(pugi::xml_node node)
{
	if (revMajor != node.attribute("revMajor").as_int()) { throw std::logic_error("revMajor not matching "); }
	//if (revMinor != node.attribute("revMinor").as_int()) { throw std::logic_error("revMinor not matching "); }
    if (node.attribute("name")) { name = node.attribute("name").as_string(); } //optional
	if (node.attribute("version")) { version = node.attribute("version").as_float(); }  //typedef
	...
	if (node.child("geoReference")) { m_geoReference = std::make_unique<t_header_GeoReference>(node.child("geoReference")); }
	if (node.child("offset")) { m_offset = std::make_unique<t_header_Offset>(node.child("offset")); }
...
t_road::t_road(pugi::xml_node node)
{
	...
	if (node.child("link")) { m_link = std::make_unique<t_road_link>(node.child("link")); }
	for (pugi::xml_node e_type = node.child("type"); e_type; e_type= e_type.next_sibling("type"))
	{
		m_types.push_back(std::make_unique<t_road_type>(e_type));
	}
	if (node.child("planView")) { m_planView = std::make_unique<t_road_planView>(node.child("planView")); }
	if (node.child("elevationProfile")) { m_elevationProfile = std::make_unique<t_road_elevationProfile>(node.child("elevationProfile")); }
	if (node.child("lateralProfile")) { m_lateralProfile = std::make_unique<t_road_lateralProfile>(node.child("lateralProfile")); }
	if (node.child("lanes")) { m_lanes = std::make_unique<t_road_lanes>(node.child("lanes")); }
	if (node.child("objects")) { m_objects = std::make_unique<t_road_objects>(node.child("objects")); }
	if (node.child("signals")) { m_signals = std::make_unique<t_road_signals>(node.child("signals")); }
	if (node.child("surface")) { m_surface = std::make_unique<t_road_surface>(node.child("surface")); }
	if (node.child("railroad")) { m_railroad = std::make_unique<t_road_railroad>(node.child("railroad")); }
   //group tag : reference g_additionalData
}
t_road_link::t_road_link(pugi::xml_node node)
{
	if (node.child("predecessor")) { m_predecessor = std::make_unique<t_road_link_predecessorSuccessor>(node.child("predecessor")); }
	if (node.child("successor")) { m_successor = std::make_unique<t_road_link_predecessorSuccessor>(node.child("successor")); }
	if (node.child("neighbor")) { m_neighbor = std::make_unique<t_road_link_neighbor>(node.child("neighbor")); }
   //group tag : reference g_additionalData
}
...
t_road_link_neighbor::t_road_link_neighbor(pugi::xml_node node)
{
	if (node.attribute("side")) { side = str2enum<e_road_link_neighbor_side>(node.attribute("side").as_string()); } // enum
    if (node.attribute("elementId")) { elementId = node.attribute("elementId").as_string(); } //required
	if (node.attribute("direction")) { direction = str2enum<e_direction>(node.attribute("direction").as_string()); } // enum
   //s_group tag : reference g_additionalData
}
t_road_type::t_road_type(pugi::xml_node node)
{
	...
	//--> ERROR <-- : if (node.attribute("country")) { country = node.attribute("country").e_countryCode; }  //union
	if (node.child("speed")) { m_speed = std::make_unique<t_road_type_speed>(node.child("speed")); }
   //group tag : reference g_additionalData
}
t_road_type_speed::t_road_type_speed(pugi::xml_node node)
{
	//--> ERROR <-- : if (node.attribute("max")) { max = node.attribute("max").t_maxSpeed; }  //union
...
t_road_planView::t_road_planView(pugi::xml_node node)
{
	...
	{
		m_geometrys.push_back(std::make_unique<t_road_planView_geometry>(e_geometry));
	}
   //group tag : reference g_additionalData
}
t_road_planView_geometry::t_road_planView_geometry(pugi::xml_node node)
{
	...
	if (node.first_child()) { m_t_road_planView_geometry = std::make_unique<t_road_planView_geometry_U>(node.first_child()); }
}
...
t_road_elevationProfile::t_road_elevationProfile(pugi::xml_node node)
{
	...
	{
		m_elevations.push_back(std::make_unique<t_road_elevationProfile_elevation>(e_elevation));
	}
   //group tag : reference g_additionalData
}
...
t_road_lateralProfile::t_road_lateralProfile(pugi::xml_node node)
{
	...
	{
		m_superelevations.push_back(std::make_unique<t_road_lateralProfile_superelevation>(e_superelevation));
	}
	for (pugi::xml_node e_crossfall = node.child("crossfall"); e_crossfall; e_crossfall= e_crossfall.next_sibling("crossfall"))
	{
		m_crossfalls.push_back(std::make_unique<t_road_lateralProfile_crossfall>(e_crossfall));
	}
	for (pugi::xml_node e_shape = node.child("shape"); e_shape; e_shape= e_shape.next_sibling("shape"))
	{
		m_shapes.push_back(std::make_unique<t_road_lateralProfile_shape>(e_shape));
	}
   //group tag : reference g_additionalData
}
...
t_road_lateralProfile_crossfall::t_road_lateralProfile_crossfall(pugi::xml_node node)
{
	if (node.attribute("side")) { side = str2enum<e_road_lateralProfile_crossfall_side>(node.attribute("side").as_string()); } // enum
	if (node.attribute("s")) { s = node.attribute("s").as_double(); }  //typedef
    if (node.attribute("a")) { a = node.attribute("a").as_double(); } //required
    if (node.attribute("b")) { b = node.attribute("b").as_double(); } //required
    if (node.attribute("c")) { c = node.attribute("c").as_double(); } //required
...
t_road_lanes::t_road_lanes(pugi::xml_node node)
{
	...
	{
		m_laneOffsets.push_back(std::make_unique<t_road_lanes_laneOffset>(e_laneOffset));
	}
	...
	{
		m_laneSections.push_back(std::make_unique<t_road_lanes_laneSection>(e_laneSection));
	}
   //group tag : reference g_additionalData
}
...
t_road_lanes_laneSection::t_road_lanes_laneSection(pugi::xml_node node)
{
	...
	if (node.child("left")) { m_left = std::make_unique<t_road_lanes_laneSection_left>(node.child("left")); }
	if (node.child("center")) { m_center = std::make_unique<t_road_lanes_laneSection_center>(node.child("center")); }
	if (node.child("right")) { m_right = std::make_unique<t_road_lanes_laneSection_right>(node.child("right")); }
   //group tag : reference g_additionalData
}
t_road_lanes_laneSection_left::t_road_lanes_laneSection_left(pugi::xml_node node)
{
	...
	{
		m_lanes.push_back(std::make_unique<t_road_lanes_laneSection_left_lane>(e_lane));
	}
   //group tag : reference g_additionalData
}
t_road_lanes_laneSection_center::t_road_lanes_laneSection_center(pugi::xml_node node)
{
	for (pugi::xml_node e_lane = node.child("lane"); e_lane; e_lane= e_lane.next_sibling("lane"))
	{
		m_lanes.push_back(std::make_unique<t_road_lanes_laneSection_center_lane>(e_lane));
	}
   //group tag : reference g_additionalData
}
t_road_lanes_laneSection_right::t_road_lanes_laneSection_right(pugi::xml_node node)
{
	for (pugi::xml_node e_lane = node.child("lane"); e_lane; e_lane= e_lane.next_sibling("lane"))
	{
		m_lanes.push_back(std::make_unique<t_road_lanes_laneSection_right_lane>(e_lane));
	}
   //group tag : reference g_additionalData
}
t_road_lanes_laneSection_center_lane::t_road_lanes_laneSection_center_lane(pugi::xml_node node)
{
	if (id != node.attribute("id").as_int()) { throw std::logic_error("id not matching "); }
	...
	if (node.child("link")) { m_link = std::make_unique<t_road_lanes_laneSection_lcr_lane_link>(node.child("link")); }
	for (pugi::xml_node e_roadMark = node.child("roadMark"); e_roadMark; e_roadMark= e_roadMark.next_sibling("roadMark"))
	{
		m_roadMarks.push_back(std::make_unique<t_road_lanes_laneSection_lcr_lane_roadMark>(e_roadMark));
	}
   //group tag : reference g_additionalData
}
t_road_lanes_laneSection_lr_lane::t_road_lanes_laneSection_lr_lane(pugi::xml_node node)
{
	...
	{
		m_materials.push_back(std::make_unique<t_road_lanes_laneSection_lr_lane_material>(e_material));
	}
	for (pugi::xml_node e_visibility = node.child("visibility"); e_visibility; e_visibility= e_visibility.next_sibling("visibility"))
	{
		m_visibilitys.push_back(std::make_unique<t_road_lanes_laneSection_lr_lane_visibility>(e_visibility));
	}
	for (pugi::xml_node e_speed = node.child("speed"); e_speed; e_speed= e_speed.next_sibling("speed"))
	{
		m_speeds.push_back(std::make_unique<t_road_lanes_laneSection_lr_lane_speed>(e_speed));
	}
	for (pugi::xml_node e_access = node.child("access"); e_access; e_access= e_access.next_sibling("access"))
	{
		m_accesss.push_back(std::make_unique<t_road_lanes_laneSection_lr_lane_access>(e_access));
	}
	for (pugi::xml_node e_height = node.child("height"); e_height; e_height= e_height.next_sibling("height"))
	{
		m_heights.push_back(std::make_unique<t_road_lanes_laneSection_lr_lane_height>(e_height));
	}
	for (pugi::xml_node e_rule = node.child("rule"); e_rule; e_rule= e_rule.next_sibling("rule"))
	{
		m_rules.push_back(std::make_unique<t_road_lanes_laneSection_lr_lane_rule>(e_rule));
	}
   //group tag : reference g_additionalData
	...
	{
		m_t_road_lanes_laneSection_lr_lanes.push_back(std::make_unique<t_road_lanes_laneSection_lr_lane_U>(e_t_road_lanes_laneSection_lr_lane));
	}
}
...
t_road_lanes_laneSection_lcr_lane_link::t_road_lanes_laneSection_lcr_lane_link(pugi::xml_node node)
{
	...
	{
		m_predecessors.push_back(std::make_unique<t_road_lanes_laneSection_lcr_lane_link_predecessorSuccessor>(e_predecessor));
	}
	for (pugi::xml_node e_successor = node.child("successor"); e_successor; e_successor= e_successor.next_sibling("successor"))
	{
		m_successors.push_back(std::make_unique<t_road_lanes_laneSection_lcr_lane_link_predecessorSuccessor>(e_successor));
	}
   //group tag : reference g_additionalData
}
...
t_road_lanes_laneSection_lcr_lane_roadMark::t_road_lanes_laneSection_lcr_lane_roadMark(pugi::xml_node node)
{
	...
	{
		m_sways.push_back(std::make_unique<t_road_lanes_laneSection_lcr_lane_roadMark_sway>(e_sway));
	}
	if (node.child("type")) { m_type = std::make_unique<t_road_lanes_laneSection_lcr_lane_roadMark_type>(node.child("type")); }
	if (node.child("explicit")) { m_explicit = std::make_unique<t_road_lanes_laneSection_lcr_lane_roadMark_explicit>(node.child("explicit")); }
   //group tag : reference g_additionalData
}
...
t_road_lanes_laneSection_lcr_lane_roadMark_type::t_road_lanes_laneSection_lcr_lane_roadMark_type(pugi::xml_node node)
{
	...
	{
		m_lines.push_back(std::make_unique<t_road_lanes_laneSection_lcr_lane_roadMark_type_line>(e_line));
	}
   //group tag : reference g_additionalData
}
...
t_road_lanes_laneSection_lcr_lane_roadMark_explicit::t_road_lanes_laneSection_lcr_lane_roadMark_explicit(pugi::xml_node node)
{
	for (pugi::xml_node e_line = node.child("line"); e_line; e_line= e_line.next_sibling("line"))
	{
		m_lines.push_back(std::make_unique<t_road_lanes_laneSection_lcr_lane_roadMark_explicit_line>(e_line));
	}
   //group tag : reference g_additionalData
}
...
t_road_lanes_laneSection_lr_lane_visibility::t_road_lanes_laneSection_lr_lane_visibility(pugi::xml_node node)
{
	if (node.attribute("sOffset")) { sOffset = node.attribute("sOffset").as_double(); }  //typedef
	if (node.attribute("forward")) { forward = node.attribute("forward").as_double(); }  //typedef
	if (node.attribute("back")) { back = node.attribute("back").as_double(); }  //typedef
	if (node.attribute("left")) { left = node.attribute("left").as_double(); }  //typedef
	if (node.attribute("right")) { right = node.attribute("right").as_double(); }  //typedef
   //s_group tag : reference g_additionalData
}
...
t_road_objects::t_road_objects(pugi::xml_node node)
{
	...
	{
		m_objects.push_back(std::make_unique<t_road_objects_object>(e_object));
	}
	...
	{
		m_objectReferences.push_back(std::make_unique<t_road_objects_objectReference>(e_objectReference));
	}
	for (pugi::xml_node e_tunnel = node.child("tunnel"); e_tunnel; e_tunnel= e_tunnel.next_sibling("tunnel"))
	{
		m_tunnels.push_back(std::make_unique<t_road_objects_tunnel>(e_tunnel));
	}
	for (pugi::xml_node e_bridge = node.child("bridge"); e_bridge; e_bridge= e_bridge.next_sibling("bridge"))
	{
		m_bridges.push_back(std::make_unique<t_road_objects_bridge>(e_bridge));
	}
   //group tag : reference g_additionalData
}
t_road_objects_object::t_road_objects_object(pugi::xml_node node)
{
    ...
    if (node.attribute("hdg")) { hdg = node.attribute("hdg").as_double(); } //required
    if (node.attribute("pitch")) { pitch = node.attribute("pitch").as_double(); } //required
    if (node.attribute("roll")) { roll = node.attribute("roll").as_double(); } //required
	...
	if (node.child("repeat")) { m_repeat = std::make_unique<t_road_objects_object_repeat>(node.child("repeat")); }
	if (node.child("outline")) { m_outline = std::make_unique<t_road_objects_object_outlines_outline>(node.child("outline")); }
	if (node.child("outlines")) { m_outlines = std::make_unique<t_road_objects_object_outlines>(node.child("outlines")); }
	for (pugi::xml_node e_material = node.child("material"); e_material; e_material= e_material.next_sibling("material"))
	{
		m_materials.push_back(std::make_unique<t_road_objects_object_material>(e_material));
	}
	for (pugi::xml_node e_validity = node.child("validity"); e_validity; e_validity= e_validity.next_sibling("validity"))
	{
		m_validitys.push_back(std::make_unique<t_road_objects_object_laneValidity>(e_validity));
	}
	if (node.child("parkingSpace")) { m_parkingSpace = std::make_unique<t_road_objects_object_parkingSpace>(node.child("parkingSpace")); }
	if (node.child("markings")) { m_markings = std::make_unique<t_road_objects_object_markings>(node.child("markings")); }
	if (node.child("borders")) { m_borders = std::make_unique<t_road_objects_object_borders>(node.child("borders")); }
   //group tag : reference g_additionalData
}
...
t_road_objects_object_outlines::t_road_objects_object_outlines(pugi::xml_node node)
{
	...
	{
		m_outlines.push_back(std::make_unique<t_road_objects_object_outlines_outline>(e_outline));
	}
   //group tag : reference g_additionalData
}
t_road_objects_object_outlines_outline::t_road_objects_object_outlines_outline(pugi::xml_node node)
{
	...
	for (pugi::xml_node e_cornerRoad = node.child("cornerRoad"); e_cornerRoad; e_cornerRoad= e_cornerRoad.next_sibling("cornerRoad"))
	{
		m_cornerRoads.push_back(std::make_unique<t_road_objects_object_outlines_outline_cornerRoad>(e_cornerRoad));
	}
	for (pugi::xml_node e_cornerLocal = node.child("cornerLocal"); e_cornerLocal; e_cornerLocal= e_cornerLocal.next_sibling("cornerLocal"))
	{
		m_cornerLocals.push_back(std::make_unique<t_road_objects_object_outlines_outline_cornerLocal>(e_cornerLocal));
	}
   //group tag : reference g_additionalData
}
...
t_road_objects_object_markings::t_road_objects_object_markings(pugi::xml_node node)
{
	...
	{
		m_markings.push_back(std::make_unique<t_road_objects_object_markings_marking>(e_marking));
	}
   //group tag : reference g_additionalData
}
t_road_objects_object_markings_marking::t_road_objects_object_markings_marking(pugi::xml_node node)
{
	...
	{
		m_cornerReferences.push_back(std::make_unique<t_road_objects_object_markings_marking_cornerReference>(e_cornerReference));
	}
   //group tag : reference g_additionalData
}
...
t_road_objects_object_borders::t_road_objects_object_borders(pugi::xml_node node)
{
	...
	{
		m_borders.push_back(std::make_unique<t_road_objects_object_borders_border>(e_border));
	}
   //group tag : reference g_additionalData
}
...
t_road_signals::t_road_signals(pugi::xml_node node)
{
	...
	{
		m_signals.push_back(std::make_unique<t_road_signals_signal>(e_signal));
	}
	...
	{
		m_signalReferences.push_back(std::make_unique<t_road_signals_signalReference>(e_signalReference));
	}
   //group tag : reference g_additionalData
}
t_road_signals_signal::t_road_signals_signal(pugi::xml_node node)
{
	...
	//--> ERROR <-- : if (node.attribute("unit")) { unit = node.attribute("unit").e_unit; }  //union
	...
	{
		m_dependencys.push_back(std::make_unique<t_road_signals_signal_dependency>(e_dependency));
	}
	for (pugi::xml_node e_reference = node.child("reference"); e_reference; e_reference= e_reference.next_sibling("reference"))
	{
		m_references.push_back(std::make_unique<t_road_signals_signal_reference>(e_reference));
	}
   //group tag : reference g_additionalData
    //t_road_signals_signal_U	m_t_road_signals_signal;
	if (node.first_child()) { m_t_road_signals_signal = std::make_unique<t_road_signals_signal_U>(node.first_child()); }
}
...
t_road_surface::t_road_surface(pugi::xml_node node)
{
	...
	{
		m_CRGs.push_back(std::make_unique<t_road_surface_CRG>(e_CRG));
	}
   //group tag : reference g_additionalData
}
...
t_road_railroad::t_road_railroad(pugi::xml_node node)
{
	...
	{
		m_switchs.push_back(std::make_unique<t_road_railroad_switch>(e_switch));
	}
   //group tag : reference g_additionalData
}
t_road_railroad_switch::t_road_railroad_switch(pugi::xml_node node)
{
	...
	if (node.child("mainTrack")) { m_mainTrack = std::make_unique<t_road_railroad_switch_mainTrack>(node.child("mainTrack")); }
	if (node.child("sideTrack")) { m_sideTrack = std::make_unique<t_road_railroad_switch_sideTrack>(node.child("sideTrack")); }
	if (node.child("partner")) { m_partner = std::make_unique<t_road_railroad_switch_partner>(node.child("partner")); }
   //group tag : reference g_additionalData
}
...
t_controller::t_controller(pugi::xml_node node)
{
	...
	{
		m_controls.push_back(std::make_unique<t_controller_control>(e_control));
	}
   //group tag : reference g_additionalData
}
...
{
    if (node.attribute("name")) { name = node.attribute("name").as_string(); } //optional
	if (node.attribute("id")) { id = node.attribute("id").as_string(); }  //typedef
	...
	{
		m_connections.push_back(std::make_unique<t_junction_connection>(e_connection));
	}
	for (pugi::xml_node e_priority = node.child("priority"); e_priority; e_priority= e_priority.next_sibling("priority"))
	{
		m_prioritys.push_back(std::make_unique<t_junction_priority>(e_priority));
	}
	for (pugi::xml_node e_controller = node.child("controller"); e_controller; e_controller= e_controller.next_sibling("controller"))
	{
		m_controllers.push_back(std::make_unique<t_junction_controller>(e_controller));
	}
	if (node.child("surface")) { m_surface = std::make_unique<t_junction_surface>(node.child("surface")); }
   //group tag : reference g_additionalData
}
t_junction_connection::t_junction_connection(pugi::xml_node node)
{
    if (node.attribute("id")) { id = node.attribute("id").as_string(); } //required
    if (node.attribute("incomingRoad")) { incomingRoad = node.attribute("incomingRoad").as_string(); } //required
    if (node.attribute("connectingRoad")) { connectingRoad = node.attribute("connectingRoad").as_string(); } //required
	if (node.attribute("contactPoint")) { contactPoint = str2enum<e_contactPoint>(node.attribute("contactPoint").as_string()); } // enum
    if (node.attribute("connectionMaster")) { connectionMaster = node.attribute("connectionMaster").as_string(); } //optional
	if (node.attribute("type")) { type = str2enum<e_junction_type>(node.attribute("type").as_string()); } // enum
	if (node.child("predecessor")) { m_predecessor = std::make_unique<t_junction_predecessorSuccessor>(node.child("predecessor")); }
	if (node.child("successor")) { m_successor = std::make_unique<t_junction_predecessorSuccessor>(node.child("successor")); }
	for (pugi::xml_node e_laneLink = node.child("laneLink"); e_laneLink; e_laneLink= e_laneLink.next_sibling("laneLink"))
	{
		m_laneLinks.push_back(std::make_unique<t_junction_connection_laneLink>(e_laneLink));
	}
   //group tag : reference g_additionalData
}
t_junction_predecessorSuccessor::t_junction_predecessorSuccessor(pugi::xml_node node)
{
	if (elementType != node.attribute("elementType").as_string()) { throw std::logic_error("elementType not matching "); }
    if (node.attribute("elementId")) { elementId = node.attribute("elementId").as_string(); } //required
	if (node.attribute("elementS")) { elementS = node.attribute("elementS").as_double(); }  //typedef
	if (node.attribute("elementDir")) { elementDir = str2enum<e_elementDir>(node.attribute("elementDir").as_string()); } // enum
   //s_group tag : reference g_additionalData
...
t_junction_surface::t_junction_surface(pugi::xml_node node)
{
	for (pugi::xml_node e_CRG = node.child("CRG"); e_CRG; e_CRG= e_CRG.next_sibling("CRG"))
	{
		m_CRGs.push_back(std::make_unique<t_junction_surface_CRG>(e_CRG));
	}
   //group tag : reference g_additionalData
}
...
{
    if (node.attribute("file")) { file = node.attribute("file").as_string(); } //required
	if (mode != str2enum<e_road_surface_CRG_mode>(node.attribute("mode").as_string())) { throw std::logic_error("mode not matching "); }
	if (node.attribute("purpose")) { purpose = str2enum<e_road_surface_CRG_purpose>(node.attribute("purpose").as_string()); } // enum
    if (node.attribute("zOffset")) { zOffset = node.attribute("zOffset").as_double(); } //optional
    if (node.attribute("zScale")) { zScale = node.attribute("zScale").as_double(); } //optional
   //s_group tag : reference g_additionalData
...
t_junctionGroup::t_junctionGroup(pugi::xml_node node)
{
	...
	{
		m_junctionReferences.push_back(std::make_unique<t_junctionGroup_junctionReference>(e_junctionReference));
	}
   //group tag : reference g_additionalData
}
...
t_station::t_station(pugi::xml_node node)
{
	...
	{
		m_platforms.push_back(std::make_unique<t_station_platform>(e_platform));
	}
   //group tag : reference g_additionalData
}
t_station_platform::t_station_platform(pugi::xml_node node)
{
	...
	{
		m_segments.push_back(std::make_unique<t_station_platform_segment>(e_segment));
	}
   //group tag : reference g_additionalData
}
...
{
	if (node.child("error")) { m_error = std::make_unique<t_dataQuality_Error>(node.child("error")); }
	if (node.child("rawData")) { m_rawData = std::make_unique<t_dataQuality_RawData>(node.child("rawData")); }
}
...
{
	if (node.child("header")) { m_header = std::make_unique<t_header>(node.child("header")); }
	for (pugi::xml_node e_road = node.child("road"); e_road; e_road= e_road.next_sibling("road"))
	{
		m_roads.push_back(std::make_unique<t_road>(e_road));
	}
	for (pugi::xml_node e_controller = node.child("controller"); e_controller; e_controller= e_controller.next_sibling("controller"))
	{
		m_controllers.push_back(std::make_unique<t_controller>(e_controller));
	}
	for (pugi::xml_node e_junction = node.child("junction"); e_junction; e_junction= e_junction.next_sibling("junction"))
	{
		m_junctions.push_back(std::make_unique<t_junction>(e_junction));
	}
	...
	{
		m_junctionGroups.push_back(std::make_unique<t_junctionGroup>(e_junctionGroup));
	}
	for (pugi::xml_node e_station = node.child("station"); e_station; e_station= e_station.next_sibling("station"))
	{
		m_stations.push_back(std::make_unique<t_station>(e_station));
	}
    //group tag : reference g_additionalData
#ifdef __DEBUG__
...
void xodr::load(std::string xodrfilename )
{
	...
	{
		std::cout << " Failed to load xml definition file .. \n" << xodrfilename.c_str();
		return;
	}
	m_root = m_doc.root();
}
void xodr::parse()
{
    ...
    try { 
        m_OpenDRIVE = std::make_unique<OpenDRIVE>(m_root.child("OpenDRIVE"));
        std::cout << " >> " << m_OpenDRIVE->m_header->revMajor << std::endl;;
        std::cout << " >> " << m_OpenDRIVE->m_roads.size() << std::endl;
    } catch (const std::exception& e) { 
        std::cerr<<"Exception on xodr::parse .. "<<e.what()<<std::endl;
    }
}
xodr::~xodr()
{
